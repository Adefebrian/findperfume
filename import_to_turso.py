"""Fast batched import: local findperfume.db -> Turso via libsql batch API."""
import sqlite3, libsql_client, asyncio, math

URL = "https://findperfume-adefebrian.aws-ap-northeast-1.turso.io"  # https for client
...
async def main():
    ...
    async with libsql_client.create_client(url=URL, auth_token=TOKEN) as client:
        ...
        for i in range(0, total, BATCH):
            ...
            # retry batch on transient errors
        ...
        print("data done, building FTS + indexes...")
        ...
        await client.execute("CREATE INDEX idx_gender ON perfumes(gender)")
        await client.execute("CREATE INDEX idx_year ON perfumes(year)")
        await client.execute("CREATE INDEX idx_pop ON perfumes(popularity DESC)")
        await client.execute("CREATE INDEX idx_scent ON perfumes(rating_scent DESC)")
        rc = await client.execute("SELECT COUNT(*) FROM perfumes")
        print("TURSO row count:", rc.rows[0][0])
